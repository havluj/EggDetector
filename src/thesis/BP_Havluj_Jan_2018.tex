...
%\usepackage[left=3.5cm,right=2.5cm,top=3.5cm,bottom=3cm]{geometry}
...
\begin{conclusion}
	...
	V další kapitole byl ètenáø seznámen se všemi nástroji, které jsou potøebné k dokonèení implementace. Navrhl a naimplementoval jsem dva plnohodnotné nástroje pro práci s daty a nespoèet drobných skriptù pro jejich zpracování. Pomocí tìchto nástrojù jsem manuálnì pøipravil \textbf{7358} trénovacích dat -- oznaèení vajec na snímcích -- které byly následnì použity k trénování neuronových sítí.
...
Výsledný konfiguraèní soubor použitý spoleènì s modelem \texttt{ssd\_mobilenet\_v1\_coco}:
\lstinputlisting[language=json]{media/pipeline.config}

\chapter{Test efektivity knihovny}\label{chap:test_efektivity}
\lstinputlisting[caption={Test k vyhodnocení efektivy knihovny.}]{media/SuccessRateTest.java}
...
\begin{figure}
	\dirtree{%
		...
		.3 \texttt{egg\_recognition}\DTcomment{zdrojové kódy implementace rozpoznávání obrazu}.
		.3 \texttt{object-detection-training}\DTcomment{zdrojové kódy implementace detekce objektù}.
