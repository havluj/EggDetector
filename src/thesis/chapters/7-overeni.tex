...
V~této kapitole se zamìøíme na testování, validaci a mìøení efektivity implementace knihovny popsané v~kapitole \ref{chap:javaknihovna}. Všechny testy jsou implementovány v~jazyce Java pomomcí testovacího frameworku JUnit~\cite{junitdoc}. Zdrojové kódy testù jsou dostupné v~pøíloze \ref{chap:cd}.

\section{Testování funkènosti}
Abychom dokázali, že souèasná implementace knihovny funguje, byly sestaveny dva \uv{jednotkové testy}\footnote{Unit testing.}. 

První test využívá pìt snímkù pøiložených ke zdrojovým kódùm testù. Nejdøíve porovnává pøedem známý poèet vajec v~tìchto snímcích s~poètem, který vypoèítá knihovna. Následnì je porovnán výsledný poèet vajec, který byl vypoèítán z~tìchto pìti snímkù.
...
Abychom otestovali, že knihovna splòuje funkèní požadavek na distribuci, vytvoøíme nový projekt, ve kterém se pokusíme knihovnu použít. 

Pøidáme knihovnu EggDetector do projektu:
...
Lze vidìt, že distribuce funguje. Zdrojové kódy externího projektu se nachází v~pøíloze \ref{chap:cd} ve složce \texttt{src/impl/libtest}.
...
Zdrojový kód testu je vypsán v~pøíloze \ref{chap:test_efektivity}. Knihovnu otestujeme pìtkrát na stejných datech s~pìti rùznými nastaveními \uv{thresholdu}. V~testech se zamìøíme na dvì metriky. První metrikou je pomìr správnì vyhodnocených složek a celkového poètu složek, vyjádøený v procentech. Tu získáme následujícím zpùsobem:
\begin{equation*}
	\boxed{
	\begin{gathered}
		\textrm{úspìšnost} = \frac{\textrm{poèet správnì vyhodnocených složek}}{\textrm{celkový poèet složek}} * 100 \%
    \end{gathered}
}
\end{equation*}
...
\begin{equation*}
	...
	\begin{gathered}
		\textrm{vzdálenost} = \\
		= \sum_{n=1}^{\textrm{poèet videosekvencí}} | \textrm{známý poèet vajec} - \textrm{poèet vajec vypoèítaný knihovnou} |
    \end{gathered}
	}
\end{equation*}

Druhá metrika je lepším nástrojem pro mìøení efektivity, protože øíká, o~kolik vajec se knihovna \uv{spletla}. Z~toho vyplývá, že \textbf{èím nižší èíslo, tím lepší efektivita}.
...
V~této kapitole bylo dokázáno, že implementace knihovny funguje a je možné ji použít formou závislostí z~jiného projektu. Mìøením efektivity knihovny jsme došli k~závìru, že nejlepším nastavením hranice pro detekci objektù je \textbf{30\%}. Knihovnu tedy upravíme tak, aby toto nastavení používala jako výchozí hodnotu.