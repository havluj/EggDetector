...
První test využívá pìt snímkù pøiložených ke zdrojovým kódùm testù. Nejdøíve porovnává pøedem známý poèet vajec v tìchto snímcích s poètem, který vypoèítá knihovna. Následnì je porovnán výsledný poèet vajec, který byl vypoèítán z tìchto pìt snímkù.
...
Zdrojový kód testu je vypsán v pøíloze \ref{chap:test_efektivity}. Knihovnu otestujeme pìtkrát na stejných datech s pìti rùznými nastaveními \uv{thresholdu}. V testech se zamìøíme na dvì metriky. První metrikou je pomìr je procentuální úspìšnosti knihovny. Tu získáme následujícím zpùsobem:
\begin{equation*}
	\boxed{
	\begin{gathered}
...
V této kapitole bylo dokázáno, že implementace knihovny funguje a je možné jíi použít formou závislostí z jiného projektu. Mìøením efektivity knihovny jsme došli k závìru, že nejlepším nastavením hranice pro detekci objektù je \textbf{30\%}. Knihovnu tedy upravíme tak, aby toto nastavení používala jako výchozí hodnotu.