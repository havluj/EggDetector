...
Skript, který jsme pøedstavili v kapitole \ref{subsec:hromadne_stazeni_dat}, stáhne veškerá data z daného serveru. Mezi takovými daty jsou snímky, které jsou pro nás užiteèné, ale zbytek stažených dat je pro nás zbyteèný. Abychom se v datech mohli lépe orientovat a ušetøili místo na pevném disku, bylo by vhodné nepotøebná data smazat. Pro tento úèel naprogramujeme jednoduchý nástroj v programovacím jazyce \texttt{Java}, který všechna nepotøebná data smaže.
...
Nástroj \texttt{Tagger} slouží k manuálnímu oznaèování snímkù. Pracuje na úrovni složek, kde jednotlivé složky a v nich umístìné snímky reprezentují jednu videosekvenci. Uživatel pak mùže program spustit a pomìrnì rychle oznaèit, kolik vajec se na daných snímcích nachází. Tagger je webovou aplikací s jednoduchým uživatelským rozhraním. Uživatel je prezentován všemi snímky v dané složky a má možnost u každého snímku specifikovat poèet vajec. Složky jsou vybírány automaticky. Jakmile uživatel uvedl poèet vajec u každého snímku, data odešle a je prezentován snímky z další složky. Tento proces se opakuje do té doby, než nebudou dostupné žádné složky s neoznaèenými snímky. Ukázka uživatelského rozhraní je vidìt na obrázku \ref{fig:tagger}.
